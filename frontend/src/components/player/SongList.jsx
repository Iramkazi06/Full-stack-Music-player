        ...
        <table className="songlist-table">
          ...
          <tbody>
            {songs.map((song, index) => (
              ...
              className={`songlist-row ${currentIndex===
              index ? "songlist-row-active":""}`}>
               <td className="songlist-td td-index">{index + 1}</td>
      {/* 1. Jamendo uses .name */}
      <td className="songlist-td">{song.name}</td> 
      {/* 2. Jamendo uses .artist_name */}
      <td className="songlist-td">{song.artist_name}</td> 
      {/* 3. Extracting Year from "2024-05-10" */}
      <td className="songlist-td">{song.releasedate?.split("-")[0] || "N/A"}</td>
